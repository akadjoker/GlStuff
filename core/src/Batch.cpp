...
void RenderBatch::Init(int numBuffers, int bufferElements)
{
    ...
    Texture2D* m_defaultTexture = TextureManager::Instance().GetDefault();
...
//             vertexBuffer[currentBuffer]->vboId[0]);
//             glBufferSubData(GL_ARRAY_BUFFER, 0,
//             vertexCounter*3*sizeof(float),
//             vertexBuffer[currentBuffer]->vertices.data());

//             glBindBuffer(GL_ARRAY_BUFFER,
//             vertexBuffer[currentBuffer]->vboId[1]);
//             glBufferSubData(GL_ARRAY_BUFFER, 0,
//             vertexCounter*2*sizeof(float),
//             vertexBuffer[currentBuffer]->texcoords.data());

//             glBindBuffer(GL_ARRAY_BUFFER,
//             vertexBuffer[currentBuffer]->vboId[2]);
//             glBufferSubData(GL_ARRAY_BUFFER, 0,
...
//             vertexBuffer[currentBuffer]->vboId[3]);
...
//                if ((draws[i]->mode == LINES) || (draws[i]->mode ==
//                TRIANGLES))
//                {
...
//                else
//                {
//                   glDrawElements(GL_TRIANGLES, draws[i]->vertexCount/4*6,
//                   GL_UNSIGNED_INT,(GLvoid *)(vertexOffset/4*6*sizeof(int)));
//                }
...
void RenderBatch::Vertex3f(float x, float y, float z)
{
    ...
    if (use_matrix)
    {
        tx = modelMatrix.x[0] * x + modelMatrix.x[4] * y + modelMatrix.x[8] * z
            + modelMatrix.x[12];
        ty = modelMatrix.x[1] * x + modelMatrix.x[5] * y + modelMatrix.x[9] * z
            + modelMatrix.x[13];
        tz = modelMatrix.x[2] * x + modelMatrix.x[6] * y + modelMatrix.x[10] * z
            + modelMatrix.x[14];
    }
...
void RenderBatch::Quad(Texture2D *texture, const Vec2 *coords,
...
void RenderBatch::Quad(Texture2D *texture, float x, float y, float width,
                       float height)
{
...
void RenderBatch::Quad(Texture2D *texture, const FloatRect &src, float x,
...
void Font::EnableClip(bool enable) { enableClip = enable; }
...
int GetCodepoint(const char *text, int *codepointSize)
{
    ...
    *codepointSize = 1;
    ...
    else if ((octet & 0xf0) == 0xe0)
    {
        ...
        {
            codepoint = ((octet & 0xf) << 12) | ((octet1 & 0x3f) << 6)
                | (octet2 & 0x3f);
            *codepointSize = 3;
        }
    }
...
void Font::drawTextCodepoint(int codepoint, float x, float y)
{
    ...
    float px = x + m_glyphs[index].offsetX * scaleFactor
        - (float)m_glyphPadding * scaleFactor;
    float py = y + m_glyphs[index].offsetY * scaleFactor
        - (float)m_glyphPadding * scaleFactor;
...
void Font::drawTexture(const FloatRect &src, float x, float y, float width,
                       float height)
{
    if (texture == nullptr)
    {
    ...
    if (enableClip)
    {
        if (quadRight < clip.x || quadLeft > clip.x + clip.width
            || quadBottom < clip.y || quadTop > clip.y + clip.height)
        {
            return;
        }
...
void Font::Print(const char *text, float x, float y)
{
    ...
    for (int i = 0; i < size;)
    {
        ...
        if (codepoint == '\n')
        {
            // NOTE: Line spacing is a global variable, use SetTextLineSpacing()
            // to setup
        ...
        else
        {
            ...
            if (m_glyphs[index].advanceX == 0)
                textOffsetX +=
                    ((float)m_recs[index].width * scaleFactor + spacing);
            else
                textOffsetX +=
                    ((float)m_glyphs[index].advanceX * scaleFactor + spacing);
        }
...
bool Font::Load(const char *filePath)
{
    ...
    std::string fontTexturePng =
        std::string(fileDir) + std::string(fileName) + std::string(".png");
    std::string fontTextureTga =
        std::string(fileDir) + std::string(fileName) + std::string(".tga");
    ...
    else if (System::Instance().FileExists(fontTextureTga.c_str()))
    {
    ...
    if (file.is_open())
    {
        ...
        while (std::getline(file, line))
        {
            ...
            // Verificamos se a linha tem o número esperado de valores separados
            // por delimitadores
            std::stringstream ss(line.substr(end + 1));
            char delimiter;
            if (ss >> delimiter >> data.x >> delimiter >> data.y >> delimiter
                >> data.width >> delimiter >> data.height >> delimiter)
            {

                if (delimiter != ',' || ss.fail() || data.width <= 1
                    || data.height <= 1)
                {
...
#define BIT_CHECK(a, b) ((a) & (1u << (b)))
    ...
    for (int i = 0; i < m_glyphCount; i++)
    {
        ...
        if (testPosX >= texture->GetWidth())
        {
            ...
            m_recs[i].y = (float)(charsDivisor
                                  + currentLine * (charsHeight + charsDivisor));
        }
        else
