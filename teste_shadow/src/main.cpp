...
void updateCascades_(float nearClip, float farClip, const Vec3& lightDirInput, const Mat4& viewMatrix, const Mat4& projectionMatrix) {
    // Normaliza a direção da luz
    Vec3 lightDir = Vec3::Normalize(lightDirInput);

    float cascadeSplits[SHADOW_MAP_CASCADE_COUNT];
    float cascadeSplitLambda = 0.95f; // Ajuste entre 0.5-0.95 se necessário
    ...
    // Calcula splits logarítmico-lineares
    ...
    for (u32 i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) {
        ...
        // Cantos do frustum em NDC (Z: -1 a 1 para OpenGL)
        ...
        for (u32 j = 0; j < 8; j++) {
            ...
            frustumCorners[j] = Vec3(invCorner.x / invCorner.w, invCorner.y / invCorner.w, invCorner.z / invCorner.w);
        }

        // Interpola para a slice da cascata
        for (u32 j = 0; j < 4; j++) {
        ...
        // Cria light view matrix
        Mat4 lightViewMatrix = Mat4::LookAt(frustumCenter + lightDir * 100.0f, frustumCenter, Vec3(0.0f, 1.0f, 0.0f));

        // Calcula AABB em light space
        float minX = FLT_MAX, maxX = -FLT_MAX;
        float minY = FLT_MAX, maxY = -FLT_MAX;
        float minZ = FLT_MAX, maxZ = -FLT_MAX;
        for (u32 j = 0; j < 8; j++) {
            Vec4 trf = lightViewMatrix * Vec4(frustumCorners[j], 1.0f);
            minX = Min(minX, trf.x); maxX = Max(maxX, trf.x);
            minY = Min(minY, trf.y); maxY = Max(maxY, trf.y);
            minZ = Min(minZ, trf.z); maxZ = Max(maxZ, trf.z);
        }

        // Estende Z moderadamente para capturar casters
        constexpr float zMult = 2.0f; // Reduzido para manter precisão
        minZ -= (maxZ - minZ) * zMult; // Estende para trás
        maxZ += (maxZ - minZ) * zMult; // Estende para frente

        // Snapping apenas no centro (como original)
        float worldUnitsPerTexel = Max(maxX - minX, maxY - minY) / float(SHADOW_WIDTH);
        Vec3 centerLS = (lightViewMatrix * Vec4(frustumCenter, 1.0f)).xyz();
        ...
        Vec3 centerWS = (Mat4::Inverse(lightViewMatrix) * Vec4(centerLS, 1.0f)).xyz();
        lightViewMatrix = Mat4::LookAt(centerWS + lightDir * 100.0f, centerWS, Vec3(0.0f, 1.0f, 0.0f));

        // Cria matriz ortográfica
        Mat4 lightOrthoMatrix = Mat4::Ortho(minX, maxX, minY, maxY, minZ, maxZ);
...
void updateCascades(float nearClip, float farClip, const Vec3 & lightPos, const Mat4 & viewMatrix, const Mat4 & projectionMatrix )
	{
		...
		for (u32 i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) 
        {
			...
			radius = Ceil(radius * TOTAL) / TOTAL;
            ...
            float orgin_z = maxExtents.z - minExtents.z;
            ...
            float worldUnitsPerTexel = (2.0f * radius) / float(SHADOW_WIDTH);
 

            // // Centro no espaço da luz
             Vec3 centerLS = (lightViewMatrix * Vec4(frustumCenter, 1.0f)).xyz();
            ...
            // // Recalcula a view com o centro "snapped"
             Vec3 centerWS = (Mat4::Inverse(lightViewMatrix) * Vec4(centerLS, 1.0f)).xyz();
             lightViewMatrix = Mat4::LookAt(centerWS - lightDir * (-minExtents.z), centerWS, Vec3(0,1,0));
...
void main()
{
    ...
    vec4 initPos = vec4(aPosition, 1.0);
    ...
    gl_Position = proj * view * model * initPos;
...
float CalculateShadow(vec4 worldPosition, int idx)
{
    ...
    // bias (constante + dependente da inclinação) –  Normal geométrica
    vec3 n = normalize(Normal);
    vec3 L = normalize(lightDir);                 //  luz é direcional
    ...
    void main() 
    {
        ...
        vec3 normalTS = texture(normalMap, TexCoords).xyz * 2.0 - 1.0;
        ...
        vec4 worlPos =  vec4(FragPos, 1.0);
        vec4 fragPosWorldSpace = view * worlPos;


        float depthValue = -outViewPosition.z;
      ...
      //  FragColor = vec4(dbg[cascadeIndex], 1.0);
        ...
        // sombrear um pouco o ambiente
         float ambientShadow = mix(1.0, 0.25, shadow); // 0.5 = 25% do ambiente nas sombras
...
if (ShaderManager::Instance().Create(depthVertexShader, depthFragmentShader, "depth"))
{
...

 {
        ...
        if (ShaderManager::Instance().Create(vertexShaderSource, fragmentShaderSource, "screen"))
        {
            Shader* depthShader = ShaderManager::Instance().Get("screen");
            depthShader->LoadDefaults();
            depthShader->SetInt("textureMap", 0);
...
int main()
{
    ...
    cubeDiffuse->SetAnisotropicFiltering(8.0f);
    ...
    Mesh* plane = MeshManager::Instance().CreatePlane(1000.0f, 1000.0f,16,16, 500, 500);
    ...
    while (device.Run())
    {
       ...
       // glBlendFuncSeparate(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ONE_MINUS_SRC_ALPHA);//good 4 text
        ...
        Shader* screenShader = ShaderManager::Instance().Get("screen");
        screenShader->Use();

        shadowManager.Bind(0);
        float y = 0.35;
        ...
        shadowManager.Bind(3);
        y = -0.7f;
        renderQuad.Render(-0.8f,y,0.3f);

        shadowManager.Bind(4);
        y = -1.0f;
      //  renderQuad.Render(-0.8f,y,0.3f);

        screenShader->Use(false);
