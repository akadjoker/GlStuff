...
bool DepthBuffer::Init(unsigned int width, unsigned int height)
{
    ...
    // Cria textura de profundidade
    glGenTextures(1, &depthMap);
    glBindTexture(GL_TEXTURE_2D, depthMap);

    //  formato suportado em ES 3.0 (recomendado 24 ou 16 bits)
    //máxima compatibilidade,  16 bits:
    ...
    // ES 3.0: sem CLAMP_TO_BORDER e sem BORDER_COLOR
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

    // (Opcional, se for amostrar como shadow map no shader com sampler2DShadow)
    ...
    // FBO só de profundidade: nenhum color buffer
